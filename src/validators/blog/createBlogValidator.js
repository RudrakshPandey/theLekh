...
export const validateCreateBlogForm = (formData) => {
  // Strip HTML from content before validation
  const cleanContent = DOMPurify.sanitize(formData.content, { ALLOWED_TAGS: [] });

  // Replace original content with plain text just for validation
  const validationData = {
    ...formData,
    content: cleanContent,
  };

  const { error } = createBlogSchema.validate(validationData, { abortEarly: false });
